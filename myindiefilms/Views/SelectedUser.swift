...
class SelectedUser: UITableViewController, commentButtonProtocol, viewProfileProtocol {
...
//        fetchUser()
    ...
    func fetchuserandsetupnavbar() {
        ...
        Database.database().reference().child("users").child((self.post?.fromID)!).observeSingleEvent(of: .value, with: { (snapshot) in
...
//
        ...
        func updatePost(){
            ...
            Database.database().reference().child("user-posts").child((self.post?.fromID)!).observeSingleEvent(of: .value, with: { (snapshot) in
                                print(snapshot)
